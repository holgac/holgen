...
bool TestLuaNumber::ParseJson(const rapidjson::Value &json, const Converter &converter, lua_State *luaState) {
  if (json.IsObject()) {
...
rapidjson::Value TestLuaNumber::DumpJson(rapidjson::Document &doc, lua_State *luaState) const {
...
void TestLuaNumber::PushToLua(lua_State *luaState) const {
  ...
  lua_settable(luaState, -3);
  lua_getglobal(luaState, "TestLuaNumber");
...
TestLuaNumber *TestLuaNumber::ReadProxyFromLua(lua_State *luaState, int32_t idx) {
  ...
  lua_pop(luaState, 1);
  HOLGEN_WARN_AND_RETURN_IF(className != CLASS_NAME, nullptr, "Received {} instance when expecting TestLuaNumber", className);
  lua_pushstring(luaState, "p");
  lua_gettable(luaState, idx - 1);
