...
#include "LuaTestStructContainerInnerType.h"
...
void LuaTestStructContainerInnerType::SetId(uint32_t val) {
  mId = val;
}
...
int LuaTestStructContainerInnerType::IndexMetaMethod(lua_State *luaState) {
  ...
  if (0 == strcmp("id", key)) {
    ...
    HOLGEN_WARN("Unexpected lua field: LuaTestStructContainerInnerType.{}", key);
    return 0;
  }
  return 1;
}
...
void LuaTestStructContainerInnerType::CreateLuaMetatable(lua_State *luaState) {
  ...
  lua_pushcfunction(luaState, LuaTestStructContainerInnerType::NewIndexMetaMethod);
  lua_settable(luaState, -3);
