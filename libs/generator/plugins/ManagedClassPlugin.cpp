...
void ManagedClassPlugin::Run() {
  for (auto &cls: mProject.mClasses) {
    ...
    auto managerField =
        manager->GetFieldFromDefinitionName(managedAnnotation->GetAttribute(Annotations::Managed_Field)->mValue.mName);
...
void ManagedClassPlugin::GenerateStaticGetter(Class &cls, const Class &manager, const ClassField &managerField) const {
  auto method = ClassMethod{St::ManagedObject_Getter, Type{cls.mName, PassByType::Pointer, Constness::NotConst},
...
void ManagedClassPlugin::GenerateIndexGetter(Class &cls, const Class &manager, const ClassField &managerField,
  ...
  auto method = ClassMethod{Naming().ManagedClassIndexGetterNameInCpp(annotation),
                            Type{cls.mStruct->mName, PassByType::Pointer, Constness::NotConst}, Visibility::Public,
  ...
  ClassMethodArgument &keyArg = method.mArguments.emplace_back("key", Type{mProject, indexedOnField->mField->mType});
