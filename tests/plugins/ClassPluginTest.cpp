...
TEST_F(ClassPluginTest, UnrecognizedMixin) {
  ...
  ExpectErrorMessage("struct A: B, C {} mixin B {}", Run, "A ({0}:1:1) uses unrecognized mixin C", Source);
  ExpectErrorMessage("struct A: B {} mixin B: C {}", Run, "B ({0}:1:16) uses unrecognized mixin C", Source);
}

TEST_F(ClassPluginTest, CircularMixin) {
  ExpectErrorMessage("struct A: A {}", Run, "Circular or duplicate mixin usage detected in A ({0}:1:1) and A ({0}:1:1)",
                     Source);
