...
int TestContainerSet::EqualsOperatorCallerFromLua(lua_State *luaState) {
  auto instance = TestContainerSet::ReadProxyFromLua(luaState, -2);
  HOLGEN_WARN_AND_RETURN_IF(!instance, 0, "Calling TestContainerSet.operator== method with an invalid lua proxy object!");
  TestContainerSet arg0Mirror;
  TestContainerSet *arg0;
  if (lua_getmetatable(luaState, -1)) {
    lua_pop(luaState, 1);
    arg0 = TestContainerSet::ReadProxyFromLua(luaState, -1);
  } else {
    arg0Mirror = TestContainerSet::ReadMirrorFromLua(luaState, -1);
    arg0 = &arg0Mirror;
  }
...
void TestContainerSet::CreateLuaMetatable(lua_State *luaState) {
  ...
  lua_pushcfunction(luaState, TestContainerSet::EqualsOperatorCallerFromLua);
  lua_settable(luaState, -3);
