...
int Country::EqualsOperatorCallerFromLua(lua_State *luaState) {
  auto instance = Country::ReadProxyFromLua(luaState, -2);
  HOLGEN_WARN_AND_RETURN_IF(!instance, 0, "Calling Country.operator== method with an invalid lua proxy object!");
  Country arg0Mirror;
  Country *arg0;
  if (lua_getmetatable(luaState, -1)) {
    lua_pop(luaState, 1);
    arg0 = Country::ReadProxyFromLua(luaState, -1);
  } else {
    arg0Mirror = Country::ReadMirrorFromLua(luaState, -1);
    arg0 = &arg0Mirror;
  }
...
void Country::CreateLuaMetatable(lua_State *luaState) {
  ...
  lua_pushcfunction(luaState, Country::EqualsOperatorCallerFromLua);
  lua_settable(luaState, -3);
